...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4">
            ...
            > {{ $t('State persistence across browser history navigation with') }} <code class="text-xs">useRemember()</code>.
            </FeatureHeader>
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <FeatureCard info-card :title="$t('Remembered Counter')">
                    <template #description> {{ $t('A separate') }} <code class="text-xs">useRemember()</code> {{ $t('instance with its own key.') }} </template>
                    <div class="space-y-4">
                        ...
                        >
                            ...
                            <p class="mt-1">
                                <code>router.remember(data, 'key')</code>{{ $t('. Save state') }} </p>
                            <p>
                                <code>router.restore('key')</code>{{ $t('. Restore state') }} </p>
                        </div>
                    </div>
                </FeatureCard>
            </div>
