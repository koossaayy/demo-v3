...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4">
            ...
            > {{ $t('Non-Inertia HTTP requests with') }} <code class="text-xs">useHttp()</code>{{ $t('. Like') }} <code class="text-xs">useForm()</code> {{ $t('but returns JSON, no page visit.') }} </FeatureHeader>
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <FeatureCard info-card :title="$t('Reactive State')">
                    <template #description> {{ $t('All the state properties from') }} <code class="text-xs">useHttp()</code>.
                    </template>
