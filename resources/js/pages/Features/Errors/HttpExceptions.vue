...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4">
            ...
            > {{ $t('Errors handled by') }} <code class="text-xs">handleExceptionsUsing()</code> {{ $t('render as full Inertia pages. Unhandled errors can be intercepted client-side with') }} <code class="text-xs">onHttpException</code>.
            </FeatureHeader>
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                >
                    <div class="space-y-4">
                        ...
                        <p class="text-xs text-muted-foreground"> {{ $t('Without interception, this shows the default error modal. With interception enabled,') }} <code>event.preventDefault()</code> {{ $t('suppresses it.') }} </p>
                    </div>
                </FeatureCard>
