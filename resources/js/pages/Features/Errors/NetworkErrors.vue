...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4">
            ...
            > {{ $t('Handle network failures with') }} <code class="text-xs">onNetworkError</code> {{ $t('callback and') }} <code class="text-xs">router.on('networkError')</code>.
            </FeatureHeader>
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <FeatureCard :title="$t('Error Interception')">
                    <template #description> {{ $t('Toggle global') }} <code class="text-xs">networkError</code> {{ $t('listener.') }} </template>
                    <div class="space-y-4">
                        ...
                        <p class="text-xs text-muted-foreground"> {{ $t('When active,') }} <code>event.preventDefault()</code> {{ $t('stops the default network error behavior.') }} </p>
