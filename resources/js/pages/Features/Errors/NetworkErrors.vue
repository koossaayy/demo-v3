...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4">
            ...
            >
                <i18n-t keypath="Handle network failures with {el0} callback and {el1}." tag="span" scope="global"><template #el0><code class="text-xs">onNetworkError</code></template><template #el1><code class="text-xs">router.on('networkError')</code></template></i18n-t>
            </FeatureHeader>
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <FeatureCard :title="$t('Error Interception')">
                    <template #description>
                        <i18n-t keypath="Toggle global {el0} listener." tag="span" scope="global"><template #el0><code class="text-xs">networkError</code></template></i18n-t>
                    </template>
                    <div class="space-y-4">
                        ...
                        <p class="text-xs text-muted-foreground">
                            <i18n-t keypath="When active, {el0} stops the default network error behavior." tag="span" scope="global"><template #el0><code>event.preventDefault()</code></template></i18n-t>
                        </p>
                        ...
                        >
                            <p>
                                <i18n-t keypath="{el0} Open DevTools → Network → set throttling to &quot;Offline&quot;, then try navigating." tag="span" scope="global"><template #el0><strong>{{ $t('To test:') }}</strong></template></i18n-t>
                            </p>
                        </div>
                    </div>
                </FeatureCard>
