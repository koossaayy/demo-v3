                    ...
                    <div class="space-y-3">
                        <div class="rounded-md bg-muted p-4 text-sm">
                            <ol class="list-inside list-decimal space-y-2">
                                <li>
                                    <i18n-t keypath="{el0}: cached data is returned immediately, no request made." tag="span" scope="global"><template #el0><strong>{{ $t('Fresh period') }}</strong></template></i18n-t>
                                </li>
                                <li>
                                    <i18n-t keypath="{el0}: stale cached data is served instantly for immediate navigation. The page updates with fresh data from the server response." tag="span" scope="global"><template #el0><strong>{{ $t('Stale period') }}</strong></template></i18n-t>
                                </li>
                                <li>
                                    <i18n-t keypath="{el0}: cache is gone, a full server request happens." tag="span" scope="global"><template #el0><strong>{{ $t('Expired') }}</strong></template></i18n-t>
                                </li>
                            </ol>
                        </div>
                    </div>
