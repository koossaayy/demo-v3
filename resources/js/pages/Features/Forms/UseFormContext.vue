...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4">
            ...
            > {{ $t('Access parent') }} <code class="text-xs">{{ $t('<Form>') }}</code> {{ $t('state from deeply nested child components. No prop drilling needed.') }} </FeatureHeader>

            <div class="grid gap-6 lg:grid-cols-2">
                <!-- Form with child components -->
                <FeatureCard :title="$t('Parent Form')">
                    <template #description> {{ $t('The') }} <code class="text-xs">{{ $t('<Form>') }}</code> {{ $t('wraps child') }} <code class="text-xs">{{ $t('FormContextField') }}</code> {{ $t('components that use') }} <code class="text-xs">useFormContext()</code> {{ $t('to access form state.') }} </template>
