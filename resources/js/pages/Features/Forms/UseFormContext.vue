...
<template>
    <Head :title="$t('useFormContext')" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4">
            <FeatureHeader
                :title="$t('useFormContext')"
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <FeatureCard :title="$t('Parent Form')">
                    <template #description> {{ $t('The') }} <code class="text-xs">&lt;Form&gt;</code> {{ $t('wraps child') }} <code class="text-xs">{{ $t('FormContextField') }}</code> {{ $t('components that use') }} <code class="text-xs">{{ $t('useFormContext()') }}</code> {{ $t('to access form state.') }} </template>
