            ...
            <div class="grid gap-6 lg:grid-cols-3">
                ...
                <FeatureCard :title="$t('Optional Prop')">
                    <template #description>
                        <i18n-t keypath="{el0}. Only loaded when explicitly requested via partial reload." tag="span" scope="global"><template #el0><code class="text-xs">Inertia::optional()</code></template></i18n-t>
                    </template>
                    <div class="space-y-3">
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-medium">{{ $t('Status') }}</span>
                ...
                <FeatureCard :title="$t('Deferred Prop')">
                    <template #description>
                        <i18n-t keypath="{el0}. Automatically loaded after the initial page render." tag="span" scope="global"><template #el0><code class="text-xs">Inertia::defer()</code></template></i18n-t>
                    </template>
                    <div class="space-y-3">
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-medium">{{ $t('Status') }}</span>
                ...
                <div class="space-y-4">
                    ...
                    >
                        ...
                        <ul class="mt-1 list-inside list-disc space-y-1">
                            <li>
                                <i18n-t keypath="{el0}: always resolved on full visits. Excluded from partial reloads unless explicitly requested." tag="span" scope="global"><template #el0><strong>{{ $t('Regular') }}</strong></template></i18n-t>
                            </li>
                            <li>
                                <i18n-t keypath="{el0}: never resolved unless explicitly requested with {el1}." tag="span" scope="global"><template #el0><strong>{{ $t('Optional') }}</strong></template><template #el1><code>only: ['optionalData']</code></template></i18n-t>
                            </li>
                            <li>
                                <i18n-t keypath="{el0}: not included on initial load, but automatically fetched client-side after render." tag="span" scope="global"><template #el0><strong>{{ $t('Deferred') }}</strong></template></i18n-t>
                            </li>
                        </ul>
                    </div>
                </div>
