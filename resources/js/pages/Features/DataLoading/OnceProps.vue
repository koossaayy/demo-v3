                    ...
                    <div class="space-y-4">
                        ...
                        >
                            <ul class="list-inside list-disc space-y-1">
                                <li>
                                    <i18n-t keypath="Navigate to Page {v0} and back. The {el0} keeps its original timestamp, the {el1} updates." tag="span" scope="global"><template #v0>{{ otherPage() }}</template><template #el0><strong>{{ $t('once prop') }}</strong></template><template #el1><strong>{{ $t('dynamic prop') }}</strong></template></i18n-t>
                                </li>
                                <li>
                                    <i18n-t keypath="{el0} uses {el1} to explicitly re-resolve once props." tag="span" scope="global"><template #el0><strong>{{ $t('Force Refresh') }}</strong></template><template #el1><code
                                        >router.reload({ only: ['staticData',
                                        'expiringData'] })</code
                                    ></template></i18n-t>
                                </li>
                                <li>
                                    <i18n-t keypath="{el0} re-evaluates on every visit. {el1} remembers for 5 seconds." tag="span" scope="global"><template #el0><strong>{{ $t('.fresh()') }}</strong></template><template #el1><strong>{{ $t('.until(5s)') }}</strong></template></i18n-t>
                                </li>
                            </ul>
                        </div>
                    </div>
