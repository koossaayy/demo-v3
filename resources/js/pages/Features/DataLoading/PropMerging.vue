            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <FeatureCard info-card :title="$t('matchOn (Dedup by ID)')">
                    ...
                    <div class="space-y-3">
                        ...
                        >
                            <p> {{ $t('Each fetch returns all known contacts with fresh timestamps plus one new contact.') }} <code>matchOn('id')</code> {{ $t('handles both at once: new IDs are appended, existing IDs are updated in place (blue highlight). After 5 fetches the pool is full, so every fetch only updates.') }} </p>
                        </div>
                    </div>
                </FeatureCard>
            </div>
