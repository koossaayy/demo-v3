...
<meta property="og:title"
  content="${dynamicTitle}" />
                ...
                <FeatureCard info-card :title="$t('Title Template')">
                    <template #description> {{ $t('Global title formatting via') }} <code class="text-xs">{{ $t('createInertiaApp') }}</code>.
                    </template>
                    <div class="space-y-3">
                        <CodeBlock>
                            <textarea>                            {{ $t('createInertiaApp({ title: (title) => title ? `${title} - ${appName}` : appName, })') }}
                            </textarea>
                        </CodeBlock>
