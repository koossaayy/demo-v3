...
function log(event: string, detail?: string) {
    eventLog.value.unshift(
        `${new Date().toLocaleTimeString(globalThis.document?.documentElement?.lang || undefined)} - ${event}${detail ? ': ' + detail : ''}`,
    );
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4">
            ...
            >
                <i18n-t keypath="Router event system with {el0}. Fires on every Inertia request." tag="span" scope="global"><template #el0><code class="text-xs">router.on()</code></template></i18n-t>
            </FeatureHeader>
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <FeatureCard info-card class="lg:col-span-2" :title="$t('Event Log')">
                    <template #description
                        ><i18n-t keypath="Events captured by {el0} listeners registered on this page." tag="span" scope="global"><template #el0><code class="text-xs">router.on()</code></template></i18n-t></template
                    >
