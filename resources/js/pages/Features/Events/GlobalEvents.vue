...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4">
            ...
            > {{ $t('Router event system with') }} <code class="text-xs">router.on()</code>{{ $t('. Fires on every Inertia request.') }} </FeatureHeader>
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <FeatureCard info-card class="lg:col-span-2" :title="$t('Event Log')">
                    <template #description
                        >{{ $t('Events captured by') }} <code class="text-xs">router.on()</code> {{ $t('listeners registered on this page.') }}</template
                    >
