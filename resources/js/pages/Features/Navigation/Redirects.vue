    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4">
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                >
                    <div class="space-y-3">
                        <p class="text-xs text-muted-foreground"> {{ $t('Uses') }} <code>{{ $t('Inertia::flash(...)') }}</code> {{ $t('then') }} <code
                                >{{ $t('to_route(\'features.navigation.redirects\')') }}</code
                ...
                >
                    <div class="space-y-3">
                        <p class="text-xs text-muted-foreground"> {{ $t('Triggers a full page visit to an external URL via') }} <code>{{ $t('window.location') }}</code>{{ $t('. The browser navigates away from the SPA entirely.') }} </p>
                        <Button
                            variant="outline"
                            size="sm"
                            @click="
