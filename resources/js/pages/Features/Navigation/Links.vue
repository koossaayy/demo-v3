...
function manualVisit(method: string) {
    ...
    if (method === 'delete') {
        fn('/features/navigation/links', {
            preserveScroll: true,
            onSuccess: () => log(t('router.{0}() completed', [method])),
        });
    } else {
        fn(
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4">
            ...
            > {{ $t('The <Link> component and programmatic router visits.') }} </FeatureHeader>
                ...
                >
                    <div class="space-y-4">
                        <div class="space-y-2">
                            ...
                            > {{ $t('Reload this page via <Link>') }} </Link>
                        </div>
