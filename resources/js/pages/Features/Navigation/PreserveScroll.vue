...
const items = Array.from({ length: 30 }, (_, i) => ({
    id: i + 1,
    title: t('Item {0}', [i + 1]),
    description: t('This is item number {0}. Scroll down to see more items and test scroll preservation.', [i + 1]),
}));
</script>
        ...
        <div class="flex h-full flex-1 flex-col gap-6 p-4">
            ...
            <FeatureCard :title="$t('Scroll Region')">
                <template #description>
                    <i18n-t keypath="This container has {el0}. Inertia tracks its scroll position separately." tag="span" scope="global"><template #el0><code class="rounded bg-muted px-1 py-0.5 text-xs"
                        >scroll-region</code
                    ></template></i18n-t>
                </template>
                <div
