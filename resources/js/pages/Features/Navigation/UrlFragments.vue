                ...
                <div class="space-y-3">
                    <div class="flex items-center gap-2">
                        ...
                        <Badge
                            ...
                            variant="default"
                            class="text-xs"
                            >{{ $t('Redirected via {0}', [redirectedFrom]) }}</Badge
                        >
                    </div>
                    <div class="flex items-center gap-2">
                ...
                >
                    <div class="space-y-4">
                        ...
                        <p class="text-xs text-muted-foreground">
                            <i18n-t keypath="The server redirects to {el0}. Check the URL bar after clicking." tag="span" scope="global"><template #el0><code class="font-mono"
...
// URL with the fragment intact"
                ...
                <FeatureCard :title="$t('Preserve Fragment')">
                    <template #description>
                        <i18n-t keypath="Navigate with a hash fragment through a server redirect. {el0} keeps the client-side fragment intact on the final URL." tag="span" scope="global"><template #el0><code class="text-xs">preserveFragment()</code></template></i18n-t>
                    </template>
                    <div class="space-y-4">
                        <div class="flex flex-wrap gap-2">
...
// Client applies #fragment from original URL"
                ...
                >
                    <template #description>
                        <i18n-t keypath="This card has {el0}. The hash redirect scrolls here." tag="span" scope="global"><template #el0><code class="text-xs">id="server-section"</code></template></i18n-t>
                    </template>
                    <p class="text-sm text-muted-foreground">
                        <i18n-t keypath="If you arrived via the hash redirect buttons above, the URL should contain {el0} and the browser scrolled to this card." tag="span" scope="global"><template #el0><code class="font-mono text-xs">#server-section</code></template></i18n-t>
                    </p>
                </FeatureCard>
                    ...
                    <div class="space-y-3 text-xs">
                        ...
                        >
                            ...
                            <p class="text-muted-foreground">
                                <i18n-t keypath="The {el0} decides the hash fragment. Redirect response includes {el1} in the URL. Middleware converts to SPA visit." tag="span" scope="global"><template #el0><em>{{ $t('server') }}</em></template><template #el1><code>#section</code></template></i18n-t>
                            </p>
                        </div>
                        <div class="rounded-md border-2 border-primary p-2">
                            <p class="font-semibold">preserveFragment</p>
                            <p class="text-muted-foreground">
                                <i18n-t keypath="The {el0} supplies the hash fragment. The server calls {el1} on the redirect to keep the original fragment on the final URL." tag="span" scope="global"><template #el0><em>{{ $t('client') }}</em></template><template #el1><code>->preserveFragment()</code></template></i18n-t>
                            </p>
                        </div>
                    </div>
                </FeatureCard>
