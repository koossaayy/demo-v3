...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4">
            ...
            > {{ $t('Navigate to a component instantly before the server responds using') }} <code class="text-xs">{{ $t('component') }}</code> {{ $t('and') }} <code class="text-xs">pageProps</code> {{ $t('options.') }} </FeatureHeader>
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <FeatureCard :title="$t('With Placeholder Props')">
                    <template #description> {{ $t('Use') }} <code class="text-xs">pageProps</code> {{ $t('callback to provide placeholder props. When') }} <code class="text-xs">pageProps</code> {{ $t('is provided, shared props are not automatically carried over, so spread them yourself.') }} </template>
                    <div class="space-y-4">
                ...
                <FeatureCard :title="$t('Callback Props')">
                    <template #description> {{ $t('Pass') }} <code class="text-xs">pageProps</code> {{ $t('as a function. Receives current page props and shared props as arguments.') }} </template>
                    <div class="space-y-4">
